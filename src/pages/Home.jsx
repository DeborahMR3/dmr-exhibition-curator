...
export default function Home() {
  ...
  const initialSearchTerm = "painting";
  ...
  async function fetchArtworks(termToSearch) {
    ...
    try {
      ...
      setItems(allResults.slice(0, PAGE_SIZE));
      if (allResults.length === 0) setError("No results found.");
    } catch (err) {
      console.error(err);
  ...
  function getMuseumSlug(object) {
    const m = (object.museum || "").toLowerCase();
    if (m.includes("met")) return "met";
    if (m.includes("chicago")) return "aic";
    return "harvard";
  }
